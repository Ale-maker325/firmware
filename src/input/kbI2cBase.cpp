    ...
    case 0x10: { // T-DECK
        ...
        // if (i2cBus->available()) {
            ...
            case 0x00: // nopress
                e.inputEvent = INPUT_BROKER_NONE;
                break;
            case 0xF2: // toggle раскладки (совместимость)
                e.inputEvent = INPUT_BROKER_LAYOUT_CHANGE;
                break;
            case 0xF3: // RP2350: принудительно EN
                // LAYOUT_CHANGE в CannedMessageModule делает toggle —
                // шлём событие только если сейчас UA, чтобы стать EN.
                if (CyrillicExtension::isCyrillic)
                    e.inputEvent = INPUT_BROKER_LAYOUT_CHANGE;
                else
                    e.inputEvent = INPUT_BROKER_NONE;
                break;
            case 0xF4: // RP2350: принудительно UA
                if (!CyrillicExtension::isCyrillic)
                    e.inputEvent = INPUT_BROKER_LAYOUT_CHANGE;
                else
                    e.inputEvent = INPUT_BROKER_NONE;
